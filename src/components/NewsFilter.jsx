...
export default function NewsFilter({
  ...
  return (
    <div className="flex flex-col lg:flex-row justify-between items-start gap-6 w-full">
      ...
      <div className="flex flex-col sm:flex-row items-start gap-3 w-full lg:w-3/4">
        {/* Tombol Filter */}
        <button
          onClick={() => setIsFilterOpen(!isFilterOpen)}
          className={`shrink-0 px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 border flex items-center gap-2 ${isFilterOpen
            ...
            >
              <motion.div
                ...
                className="
                  ...
                  px-2
                "
              /* PERBAIKAN: Menghapus md:flex-nowrap agar kategori bisa membungkus (wrap) ke bawah.
                 Margin mr-1 mb-1 di tombol juga dihapus dan diganti full menggunakan gap-2.5 agar rapi */
              >
                {categories.map((category) => (
                  <motion.button
                    ...
                    className={`shrink-0 px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 border ${selectedCategory === category
                        ? "bg-white inset-shadow-xl text-[#111827] border-transparent shadow-[0_0_7px_3px_rgba(0,0,0,0.1)]"
